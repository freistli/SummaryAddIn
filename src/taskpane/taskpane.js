...
export async function summary() {
  ...
  const url = config.summary_endpoint + selectedValue;
...
export async function translate() {
  ...
  // Record the start time
  var startTime = Date.now();
  document.getElementById("item-content").value = "Loading.......";
  const url = config.translate_endpoint + selectedValue;
  ...
  const response = await fetch(url, {
    ...
    .then((data) => {
      ...
      document.getElementById("item-content").value = 'Loaded in ' + timeTaken + ' seconds. \r\n' + data;
    })
    .catch((error) => document.getElementById("item-content").value = 'Loaded in ' + timeTaken + ' seconds. \r\n' + error);
}
